...
function fields(m={}){let allowed=new Set(m.allowed_mcp_servers||[]),mcps=[...new Set(S.engines.flatMap(e=>e.mcp_servers||[]))];return `<label>Name<input name="name" value="${esc(m.name)}" required></label><label>Kind<select name="kind"><option value="agent">Agent</option><option value="human" ${m.kind==='human'?'selected':''}>Human lead</option></select></label><label>Engine<select name="engine"><option value="">—</option>${engines().map(e=>`<option value="${e.id}" ${m.engine===e.id?'selected':''}>${esc(e.name)}</option>`).join('')}</select></label><label>Model<input name="model" value="${esc(m.model)}" placeholder="Engine default"></label><label>Effort<input name="effort" value="${esc(m.effort)}" placeholder="Engine default"></label>${mcps.length?`<div class="field">Allowed MCP servers<div class="mcp-list">${mcps.map(name=>`<label class="choice"><input type="checkbox" name="allowed_mcp_servers" value="${esc(name)}" ${allowed.has(name)?'checked':''}> ${esc(name)}</label>`).join('')}</div></div>`:''}${m.id&&m.kind!=='human'?'<label class="danger"><input type="checkbox" name="remove"> Remove member</label>':''}`}
function edit(m){
  $('title').textContent=m?'Edit member':'New member';$('fields').innerHTML=fields(m);let d=$('dialog');d.showModal();
  d.onclose=async()=>{if(d.returnValue!=='default')return;let f=new FormData(d.querySelector('form'));try{if(f.get('remove'))await api('/api/members/'+m.id,{method:'DELETE'});else{let v={name:f.get('name'),kind:f.get('kind'),engine:f.get('engine')||null,model:f.get('model')||null,effort:f.get('effort')||null,allowed_mcp_servers:f.getAll('allowed_mcp_servers')};await api('/api/members'+(m?'/'+m.id:''),{method:m?'PATCH':'POST',body:JSON.stringify(v)})}load()}catch(e){alert(e.message)}};
}
